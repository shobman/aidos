...
- **AIDOS GitHub MCP Connector (Claude Desktop).** Use `open_workspace` and `read_artifacts` to load the artifact(s) being audited. You are read-only — never call `save`, `publish`, `resolve`, or any write tool.
